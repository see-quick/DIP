...
(b) the next part will be production based experiments (i.e., acceptance), which will primarily provide information on whether it is beneficial
...
V tejto sekcií zahrnieme dva rôznorodé experimenty.
V Sekcií~\ref{06:subsec:acceptance-experiments} budú popísané
a ukázané experimenty nad malou produkčnou sadou system testov.
Zároveň tak zistíme, či je pre malú sadu testov parallelizácia aplikovateľná.
V dalšej časti (i.e, Section~\ref{06:subsec:regression-experiments}) získame na druhú stranu iný pohľad nad veľkou sadou system testov, ktorou aktuálne disponuje Strimzi.
...
nástroji.
Výsledok týchto testov nám zároveň dodá informáciu, či je ideálne púštať paralelne.
Kedže viem, že daný profil obsahuje aj dosť testov pre ktoré je nutná izoláciu (i.e., @IsolatedTest) a rovnako tak aj testovacie triedy (i.e., @IsolatedSuite).
V tomto ohlade sa teda čaká podstatne slabšie zrýchlenie ak to porovnáme z preliminárnymi experimentami, ktoré sa púštali
s takmer najlepšom prostredí pre paralelizáciu.
Zároveň kedže sme zistili, že flavour \emph{2CPUs a 8GB RAM}, nie je možný vykonávať ani dve paralelné testy a teda je pre tento typ experimentu zbytočný.

% TODO:     1. popisat Amdalh zakon na jednotlive instancie MINIKUBE co ocakame ake teoreticke zrychlenie
%           2. Nasledne popisat vysledky ze sa vlastne dalo cakat ze to nebude velke zrychlenie kvoli poctu IsolatedTest...

\begin{table}[ht!]
    \centering
    \includegraphics[scale=0.8]{obrazky-figures/08-experiments/acceptance/06-exp-acceptance-minikube}
    \caption{Experiments aimed at class-wide parallelisation and one execution for method-wide by which we compare these
    two approaches and found non-correlation. Overall thirty test cases were executed (i.e., nineteen @ParallelTest,
        ten @ParallelNamespaceTest and one@IsolatedTest).
    }
...
% TODO:     1. popisat Amdalh zakon na jednotlive instancie OCP co ocakame ake teoreticke zrychlenie
%           2. Nasledne popisat vysledky ze sa vlastne dalo cakat ze to nebude velke zrychlenie kvoli poctu IsolatedTest...avsak je vidiet nejake zrychlenie...
%    ------
% final words = kedze dane testy trvaju iba 2h tak tam to zrychlenie nie je vidiet avsak v nasledujucich experintoch budeme vidiet
% zrychlenie v meritku hodin co je urcite priatelne a skvele zistenie...

\begin{table}[ht!]
    \centering
    \includegraphics[scale=0.8]{obrazky-figures/08-experiments/acceptance/06-exp-acceptance-ocp}
    \caption{Experiments aimed at class-wide parallelisation and one execution for method-wide by which we compare these
    two approaches and found non-correlation. Overall thirty test cases were executed (i.e., nineteen @ParallelTest,
        ten @ParallelNamespaceTest and one@IsolatedTest).
    }
