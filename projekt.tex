...
\projectinfo{
  ...
  abstract.cs={
        ...
        Táto technológia otvorila mnoho nových možností nielen pre veľké spoločnosti, ale aj pre malých vývojárov softvéru.
        ...
        Aby sme tento problém vyriešili, prijali sme princípy paralelného vykonávania a vytvorili mechanizmus v rámci systémových testov Strimzi, ktorý paralelne spúšťa testy iba proti jedinému klastri Kubernetes.
...
% I have listed all the literary sources, publications and other sources, which were used during the preparation of this thesis.},
  ...
  acknowledgment={
        I would like to thank my supervisors, Ing. Jakub Stejskal, Doc. Milan Češka and Dr. Thomas Cooper PhD for
        their time. This work is realized in cooperation with Red Hat Czech, s.r.o.},
...
%(external submitter, consultant, etc.).},
  ...
  extendedabstract={
        ...
        \emph{Kubernetes}~\cite{história, kubernetes, kubernetesBook} je systém na správu kontajnerov a v ďalšej miniiterácii priniesol nový koncept, ako efektívnejšie organizovať kontajnery \---\ \emph{vzor operátora }.
        ...
        Preto je jednou z najväčších výziev používania \emph{Kubernetes} efektívne a rýchle testovanie projektov ako \emph{Kafka} a \emph{Strimzi} pri overovaní integrácie s podobnými produktmi (t.j.
        \emph{Prometheus}\footnote{Prometheus \---\ open-source projekt založený na metrikách. Navyše poskytuje výstražný systém s neuveriteľnými funkciami, v prípade záujmu \url{https://prometheus.io/}},
        Grafana\footnote{Grafana \---\ open-source projekt, ktorého primárnou zodpovednosťou je ukázať používateľovi interaktívnu vizualizáciu na sledovanie kľúčových častí systému prostredníctvom skvelého používateľského rozhrania. (\url{https://grafana.com/})},
        Jaeger\footnote{Jaeger (Jaeger Tracing) \---\ je produkt, ktorý hľadá a pomáha pri riešení problémov v distribučných systémoch. (\url{https://www.jaegertracing.io/})},
        Keycloak\footnote{Keycloak \---\ open-source projekt na zabezpečenie aplikácií (autentifikácia a autorizácia). (\url{https://www.keycloak.org/})}).
        ...
        Pozorný čitateľ mohol vidieť celý čas testu blížiaci sa k tisícke hodín, čo je približne jeden a pol mesiaca.

        Napriek tomu, ako súčasť tohto úsilia o hrubozrnný paralelizmus pri vykonávaní viacerých nasadení produktov, čiastočne urýchlil celkový výpočet.
        Tento prístup však nie je horizontálne škálovateľný kvôli našim cloudovým službám, ktoré poskytujú zdroje (t. j. holé kovy, virtuálne stroje, kontajnery).
        ...
        Tieto informácie nás motivovali navrhnúť a implementovať mechanizmus jemnozrnného paralelizmu v našom testovacom rámci.

        Táto práca sa zaoberá paralelizáciou testov systému Kubernetes Strimzi.
        Autor prispel daným kódom do open-source projektu Strimzi dostupného na Github\footnote{Strimzi Github repository \---\ \url{https://github.com/strimzi/strimzi-kafka-operator}}, ktorý tiež umožňuje inšpirovať ostatných \emph{kube-native}\footnote{Kube-native \---\ je to produkt, ktorý bol presunutý zo samostatného sveta do sveta Kubernetes. Okrem toho poskytuje komunikačné rozhranie (t. j. Kubernets REST API), pomocou ktorého spravuje jednotlivé komponenty (t. j. Apache Kafka je samostatná aplikácia a Strimzi je \emph{kube-native} produkt, pretože zahŕňa Apache Kafka a poskytuje komunikačné rozhranie pre používateľa.} produkty na implementáciu takýchto riešení.
        Komplexným prínosom tejto práce je výrazné zrýchlenie procesu overovania.
    },
